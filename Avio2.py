...
class JocAvions(ShowBase, DirectObject):
	def __init__(self):
		...
		#~ self.terreny = self.terreny.find("**/terreny")
		...
		def mouBala(bala, balaImage, vel, queue, task):		
			if task.time > 3:
				expbala = self.loader.loadModel("prova/textures/expbala.egg")
				expbala.reparentTo(self.particlesDummy)
				expbala.setBillboardPointEye()
				expbala.setPos(bala.getPos(self.particlesDummy))
				bala.detachNode()
				taskMgr.doMethodLater(.2, suprimir, "suprimir", extraArgs = [expbala])
				print "bum"	
				return task.done
			if queue.getNumEntries()!=0:
				...
				expbala.reparentTo(self.particlesDummy)
				expbala.setBillboardPointEye()
				expbala.setPos(entry.getSurfacePoint(self.particlesDummy))
				taskMgr.doMethodLater(.2, suprimir, "suprimir", extraArgs = [expbala])
				print "bum", bala.getPos()	
				return task.done
			bala.setFluidPos(bala, 0, vel*globalClock.getDt(), 0)
			balaImage.setR(balaImage, 1000*globalClock.getDt())
			return task.cont

		def suprimir(expbala):
			expbala.detachNode()
